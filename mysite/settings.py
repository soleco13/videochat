...
CHANNEL_LAYERS = {
    "default": {
        ...
        "CONFIG": {
            ...
            # КРИТИЧНО: Увеличено для поддержки 10 пользователей (много ICE кандидатов)
            "capacity": 20000,  # Максимум сообщений в канале (увеличено для 10 пользователей)
            # ⚠️ ВАЖНО: Баланс между очисткой и достаточным временем для обработки
            "expiry": 15,  # Удаляем сообщения через 15 секунд (увеличено для стабильности)
            # Очистка групп
            "group_expiry": 180,  # Группы живут 180 секунд после последней активности (увеличено)
            # Дополнительные настройки для производительности
            "channel_capacity": {
                "http.request": 5000,
                "http.response!*": 5000,
                "websocket.send": 20000,  # КРИТИЧНО: Увеличено для поддержки 10 пользователей
            },
        },
    },
}
